...
double cbe; //for break
double cbp; //for break

inline void pickAndFlipNCModified() {
	register int i, j;
	int bestVar;
	int rClause, tClause;
	rClause = falseClause[flip % numFalse]; //random unsat clause
...
void setupParametersModified() {
	if (maxClauseSize <= 3){
		pickAndFlipVar = pickAndFlipNCModified; //no caching of the break values in case of 3SAT
		caching =0;
	}
	if (!cb_spec) {
		if (maxClauseSize <= 3) {
...
void initModified() {
	...
	probsBreak = (double*) malloc(sizeof(double) * (maxNumOccurences + 1));
	for (i = 0; i <= maxNumOccurences; i++) {
		probsBreak[i] = pow((eps + i), -cbp)*pow(cbe, -i);
	}
}
...
inline void allocateMemoryCD() {
	// Allocating memory for the instance data (independent from the assignment).
	numLiterals = numVars * 2;
	atom = (char*) malloc(sizeof(char) * (numVars + 1));
	C = (double*) malloc(sizeof(double) * (numVars + 1));
	//clause = (int**) malloc(sizeof(int*) * (numClauses + 1));
	numOccurrence = (int*) malloc(sizeof(int) * (numLiterals + 1));
	occurrence = (int**) malloc(sizeof(int*) * (numLiterals + 1));
	critVar = (int*) malloc(sizeof(int) * (numClauses + 1));
...
double cattack,cbase2,cbase1;
double crealm;

inline void CDFlip(int tClause, int var, double attack)
{
	double e = atom[var] ? (1-C[var]) : C[var];
	e*= attack;
	...
	//flip bestvar
	if (atom[var])
		xMakesSat = -var; //if x=1 then all clauses containing -x will be made sat after fliping x
	else
		xMakesSat = var; //if x=0 then all clauses containing x will be made sat after fliping x
	atom[bestVar] = 1 - atom[var];
	//1. Clauses that contain xMakeSAT will get SAT if not already SAT
	//numOccurenceX = numOccurrence[numVars + xMakesSat];
	int i = 0;
	while ((tClause = occurrence[xMakesSat + numVars][i])) {
...
inline void pickAndFlipNCCD() {
	...
	while ((lit = clause[rClause][i])) {
		...
		double e = lit==0 ? (1-C[i]) : C[i];
		probs[i] = probsBreak[breaks[i]]*pow(e,ce);
		sumProb += probs[i];
		i++;
	}
	double sum = sumProb;
	randPosition = (double) (rand()) / RAND_MAX * sum;
	for (i = i - 1; i != 0; i--) {
		sum -= probs[i];
		if (sum <= randPosition)
			break;
	...
	secondVar = abs(clause[rClause][i]);
	...
	thirdVar = abs(clause[rClause][i]);

	CDFlip(tClause,bestVar,cattack);
	CDFlip(tClause,secondVar,cbase2);
	CDFlip(tClause,thirdVar,cbase1);
}

void setupParametersCD() {
	if (maxClauseSize <= 3){
		pickAndFlipVar = pickAndFlipNCCD; //no caching of the break values in case of 3SAT
		caching =0;
	}
	if (!cb_spec) {
		if (maxClauseSize <= 3) {
